...
import { FolderGit, History, Home, LogOut, MessagesSquare, Package2, PencilRuler, User, PanelLeft } from 'lucide-react'
...
const HomePage = () => {
  ...
  const { message, error, user, isAuthenticated } = useSelector((state) => state.user)
  const handleLogout = () => {
    dispatch(logout())
  }
  useEffect(() => {
    ...
    <>
      <div className='flex min-h-screen w-full flex-col bg-muted/40'>
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6 max-[900px]:h-[100px]">
          ...
          <div className="flex items-center gap-4 md:grow-0 sm:ml-16 sm:mt-5 object-contain">
            <img
              src={user && user.avatar && user.avatar.url}
              alt="avatar"
              className="w-20 h-20 object-cover rounded-full max-[900px]:hidden"
            />
