...
export const getAllTimeline = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${backendUrl}/api/v1/timeline/getAll`,
      { withCredentials: true }
    );
    dispatch(
      timelineSlice.actions.getAllTimelineSuccess(response.data.allTimeline)
    );
...
export const addNewTimeline = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${backendUrl}/api/v1/timeline/add`,
      data,
      {
        withCredentials: true,
...
export const deleteTimeline = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${backendUrl}/api/v1/timeline/delete/${id}`,
      {
        withCredentials: true,
      }
    );
